 ...
 */
export function breadthFirstSearch(fromNode, goalNode, graph){

  if(!(0 < fromNode && fromNode < graph.length) || !(0 < toNode && toNode < graph.length)){
    //Throw something
  }

  var q = new Queue(graph.length);//Using a ring buffer queue
  ...
  var Node = function(nodeNr, cameFrom, NrOfHops){
  ...
  while(!q.isEmpty()){
    ...
    for(var adjacent = 0; adjacent < graph.length; adjacent++){
      if((graph[currentNode.nodeNr][adjacent] < 0) || visited[currentNode.nodeNr]){
          continue;
      }
